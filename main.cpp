...
vector<int> Tabuleiro::ataqueDosCavaleiros() {
	...
	for (unsigned int i = 0; i < this->verticesCavaleiros.size(); i++) {
	...
	for (unsigned int i = 0; i < caminhoCavalos.size(); i++) {
		if (menor > caminhoCavalos.at(i)) {
			menor = caminhoCavalos.at(i);
		}
	}

	for (unsigned int j = 0; j < caminhoCavalos.size(); j++) {
...
void Tabuleiro::setPosCavalos(vector<string> posCavalos) {
	...
	for (unsigned int i = 0; i < posCavalos.size(); i++) {
